...
void RegisterUser() {
    ...
    while (1) {
        ...
        // User Defined Function for Exiting Midway
        if (ExitFunction(EnteredNumber) == 1){      	
   			return;
		}
        ...
        if (valid == 1) {
        	NewAccNum = abs(atoi(EnteredNumber));
            break;
        } else {
    ...
    if (accountExists == true) {
        ...
        printf(YELLOW BOLD "================================\n" RESET);
        printf(RED "Might Have Changed Your Original Input to Prevent Scam/Fraud\n");
...
void LoginUser() {
    ...
    while (1) {
        char strAN[11]; // Increased to 11 to accommodate 10 digits plus null terminator
        ClearScreen();
        printf(BLUE BOLD "=========================\n" RESET);
...
void ListTransact() {
	...
	while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
	    ...
	    if (strcmp(transaction.TransactionCode, "D") == 0) {
	        printf(WHITE " %-20d "MAGENTA"|"WHITE" %-25s "MAGENTA"| " GREEN "%-6s" MAGENTA "  |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Deposit", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    } else if (strcmp(transaction.TransactionCode, "W") == 0) {
	        printf(WHITE " %-20d "MAGENTA"|"WHITE" %-25s "MAGENTA"| " RED "%-6s" MAGENTA " |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Withdraw", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    } else {
	        printf(WHITE " %-20d "MAGENTA"|"WHITE" %-25s "MAGENTA"| " YELLOW "%-6s" MAGENTA " |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Unknown", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    }
	}
...
void SearchData(int AN, char *FN, char *LN, char *Month, char *Date, char *Year) {
    ...
    while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
        ...
        if (match == 1) {
            ...
            printf(WHITE " %-20d " MAGENTA "|" WHITE " %-25s " MAGENTA "| " WHITE "%-6s" MAGENTA " |" WHITE " %-15s " MAGENTA "|"WHITE" %-12s " MAGENTA "| " WHITE "%-10.2lf " MAGENTA "|\n" RESET,
