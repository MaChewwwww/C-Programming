...
void RegisterUser() {
    ...
    char EnteredNumber[11]; // increased size to accommodate null terminator
    while (1) {
        printf(YELLOW "\nEnter Account Number: " RESET);
        scanf("%10s", EnteredNumber); // limit input to 10 characters
        clearInputBuffer();
        ...
        if (valid == 1) {
            strcpy(newUser.AccountNumber, EnteredNumber);
            break;
        } else {
            printf(RED "\nPlease Enter a 10-digit Numeric Account Number!\n" RESET);
        }
    }
    ...
    while (fread(&newUser, sizeof(struct PendingUser), 1, p_userFile)) {
        if (strcmp(newUser.AccountNumber, EnteredNumber) == 0) {
            accountExists = true;
            break;
        }
    }
    ...
    while (fread(&existingUser, sizeof(struct RegisteredUser), 1, userFile)) {
        if (existingUser.AccountNumber == atoi(EnteredNumber)) {
            accountExists = true;
            break;
        }
    }
    ...
    if (accountExists) {
        ...
        clearInputBuffer(); // Clear any remaining characters including newline
        printf(YELLOW "Enter First Name: " RESET);
        scanf(" %[^\n]", newUser.FirstName); // Read full name with spaces
        if (ExitFunction(newUser.FirstName) == 1) {
            return;
        }
        
        clearInputBuffer(); // Clear any remaining characters including newline
        printf(YELLOW "Enter Last Name: " RESET);
        scanf(" %[^\n]", newUser.LastName); // Read full name with spaces
        if (ExitFunction(newUser.LastName) == 1) {
            return;
        }
        ...
        char strpin[7]; // increased size to accommodate null terminator
        while (1) {
            ...
            if (pinValid) {
                ...
                printf(RED "\nPlease enter a 6-digit numeric Pin!\n" RESET);
            }
        }
        ...
        while (newUser.AccountBalance != 100.0) {
            ...
            float deposit = strtof(strpin, NULL);
    ...
    while (1) {
            ...
            isdigit(strAN[9])) 
        {
            ...
            while (fread(&user, sizeof(struct RegisteredUser), 1, userFile)) {
                if (user.AccountNumber == atoi(strAN)) {
                    accExists = true;
                    break;
                }
                else
        ...
        while (1) {
            ...
            if (strlen(input) == 6 && 
                ...
                if (user.AccountPin == enteredPin) {
                    ...
                    int transactionResult = Transaction(user.AccountNumber, enteredPin);
...
void ApprovePendingUser() {
    ...
    do {
        ...
        if (p_userFile == NULL || userFile == NULL || tempFile == NULL) {
            printf(RED "Error opening file!\n" RESET);
            return;
        }
        ...
        while (fread(&pendingUser, sizeof(struct PendingUser), 1, p_userFile)) {
            printf(GREEN "Account Number: " WHITE "%s\n" RESET, pendingUser.AccountNumber);
            printf(GREEN "Name: " WHITE "%s %s\n\n" RESET, pendingUser.FirstName, pendingUser.LastName);
        }
        printf(GREEN BOLD "========================================\n" RESET);
        ...
        while (fread(&pendingUser, sizeof(struct PendingUser), 1, p_userFile)) {
            if (strcmp(pendingUser.AccountNumber, accountNumber) == 0){
                found = 1;
                break;
                // Copy all fields from pendingUser to newUser
                newUser.AccountNumber = atoi(pendingUser.AccountNumber); 
                strcpy(newUser.FirstName, pendingUser.FirstName); // Copy first name
                strcpy(newUser.LastName, pendingUser.LastName);   // Copy last name
                ...
                // Write newUser to userFile
                fwrite(&newUser, sizeof(struct RegisteredUser), 1, userFile);
            } else {
                // Write pendingUser to tempFile if it's not the approved user
...
void DenyPendingUser() {
    ...
    do {
        ...
        while (fread(&pendingUser, sizeof(struct PendingUser), 1, p_userFile)) {
            if (strcmp(pendingUser.AccountNumber, accountNumber) == 0){
                found = 1;
                deniedUser.AccountNumber = atoi(pendingUser.AccountNumber);
...
void FilterData() {
    char enteredAN[10] = "NULL"; //1231231231
