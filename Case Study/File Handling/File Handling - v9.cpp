...
#include <conio.h>      // For console input/output (getch)
#include <stdlib.h>     // For general utilities (atoi, strtof, strtod, exit)
#include <string.h>     // For string operations (strcpy, strcmp, strlen)
#include <stdbool.h>    // For Boolean type and values (bool, true, false)
#include <ctype.h>      // For character checks (isdigit)
...
// Structure for a registered user = accounts.dat
...
void ListTransact();
void FilterData();
void SearchData(char *AN, char *FN, char *LN, char *Month, char *Date, char *Year);
int Transaction(char *AccountNumber, int EnteredPin);
...
void AdminMenu() {
    ...
    while (1) { // Infinite loop until correct password is entered or 'x' is entered
        ...
        printf(MAGENTA BOLD "=================================\n" RESET);
        printf(WHITE BOLD "Press X to Cancel\n" RESET);
        printf(MAGENTA BOLD "Enter Admin Passcode : " RESET);
        scanf("%s", input_pass);
...
void RegisterUser() {
    ...
    // Open or create the users file in read/write mode to manage existing accounts
	...
	// Open or create the pending users file in append mode to handle new registrations
	p_userFile = fopen("pending_users.dat", "a+b");
	if (p_userFile == NULL) {
	    printf(RED "Error opening pending users file!\n" RESET);
	    fclose(userFile); // Close previously opened users file
	    getch();
	    return;
	}
    ...
    UserCount += 1; //Unique Account Number of new User
    sprintf(EnteredNumber, "%010d", UserCount); // Convert to String and Format the Account Number as a 10-digit string with leading zeros
    strcpy(newUser.AccountNumber, EnteredNumber); // Save Data to Data Structure using strcpy
    ...
    if (accountExists == true) {
        ...
        if (ExitFunction(enteredFirstName) == 1){
   			return;
		}
        ...
        clearInputBuffer();
        if (ExitFunction(enteredLastName) == 1){
   			return;
		}
		clearInputBuffer();
        ...
        if (ExitFunction(newUser.AccountPassword) == 1){
   			return;
		}
        ...
        while (1) {
            ...
            if (strlen(strpin) == 6 && 
			    ...
			    isdigit(strpin[5]) != 0) 
				{
	                ...
	                break;
					// Alternative Way to check for length and if its a digit
			        //if (length != 10) {
			            //valid = 0;
			        //} else {
        ...
        newUser.AccountBalance = 0.0;
		char enteredInitialDeposit[10]; 		
		while (newUser.AccountBalance != 100.0) {
		    printf(YELLOW "Enter Initial Deposit (Exactly 100php): " RESET);
		    scanf("%s", enteredInitialDeposit);
		
		    if (ExitFunction(enteredInitialDeposit) == 1) {
		        break; // Exit function if required
		    }
		
		    // Convert enteredInitialDeposit to float using strtof
		    double deposit = strtod(enteredInitialDeposit, NULL);
		
		    // Check if the conversion was successful and the deposit is exactly 100 PHP
		    if (deposit == 100.0) {
		        newUser.AccountBalance = deposit;
		        break; // Exit the loop since the correct deposit is entered
		    } else {
		        printf(RED "\nPlease deposit Exactly 100php!\n" RESET);
		    }
		}
...
void LoginUser() {
    ...
    char input[7]; // Increased to 7 to accommodate 6 digits plus null terminator
    int checkpass = 0;
    int checkpin = 0;
    int foundUser = 0;
    ...
    pendingFile = fopen("pending_users.dat", "r+b");
    if (pendingFile == NULL) {
        printf(RED "Error opening pending users file!\n" RESET);
        fclose(userFile);
        return;
    ...
    // Control Structure to ensure that the number entered is 10 DIGIT ONLY. Does not allow alphabet and Does not allow Special Characters
    while (1) {
        ClearScreen();
        printf(BLUE BOLD "=========================\n" RESET);
        ...
        if (strcmp(strAN, "x") == 0 || strcmp(strAN, "X") == 0) {
            ...
            isdigit(strAN[0]) && isdigit(strAN[1]) && isdigit(strAN[2]) &&
            isdigit(strAN[3]) && isdigit(strAN[4]) && isdigit(strAN[5]) &&
            isdigit(strAN[6]) && isdigit(strAN[7]) && isdigit(strAN[8]) && 
            isdigit(strAN[9])) 
        {
            ...
            while (fread(&user, sizeof(struct RegisteredUser), 1, userFile)) {
                if (strcmp(user.AccountNumber, strAN) == 0) {
                    accExists = true;
                    break;
                }
                else
    ...
    } // End of control structure for account number

    // Close pending file as it's no longer needed
    fclose(pendingFile);
    
    if (!accExists) {
        printf(RED "\nThe Account Number that you have entered is not Registered Yet!\n" RESET);
        printf(YELLOW "(Press Any Key to Continue)\n" RESET);
        getch();
        fclose(userFile);
        return;
        ...
        while (1) {
            ...
            if (strlen(input) == 6 && 
                ...
                if (user.AccountPin == enteredPin) {
                    ...
                    int transactionResult = Transaction(strAN, enteredPin);
...
void ApprovePendingUser() {
    ...
    do {
        ...
        while (fread(&pendingUser, sizeof(struct PendingUser), 1, p_userFile)) {
            if (strcmp(pendingUser.AccountNumber, enteredAN) == 0) {
                found = 1;
                strcpy(newUser.AccountNumber, pendingUser.AccountNumber);
        ...
        printf(WHITE BOLD "\nDo you want to approve another user? ("GREEN"Y"WHITE BOLD"/"RED"N"WHITE BOLD"): " RESET);
...
void DenyPendingUser() {
    ...
    do {
        ...
        while (fread(&pendingUser, sizeof(struct PendingUser), 1, p_userFile)) {
            if (strcmp(pendingUser.AccountNumber, enteredAN) == 0) {
                found = 1;
                strcpy(deniedUser.AccountNumber, pendingUser.AccountNumber);
        ...
        printf(WHITE BOLD "\nDo you want to deny another user? ("GREEN"Y"WHITE BOLD"/"RED"N"WHITE BOLD"): " RESET);
        scanf(" %c", &choice); // Note the space before %c to consume the newline character
...
int Transaction(char *AccountNumber, int EnteredPin) {
    ...
    while (fread(&user, sizeof(struct RegisteredUser), 1, userFile)) {
        if (strcmp(user.AccountNumber, AccountNumber) == 0  && user.AccountPin == EnteredPin) {
            foundUser = true;
            break;
        }
    }
    ...
    while (quit == 0) {
        ...
        while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
            if (strcmp(user.AccountNumber, AccountNumber) == 0 && (strcmp(transaction.TransactionCode, "D") == 0))
                TotalDeposit += transaction.Amount;
            else if (strcmp(user.AccountNumber, AccountNumber) == 0 && (strcmp(transaction.TransactionCode, "W") == 0))
        ...
        switch (choice) {
            case 1: // Deposit
				...
				//}
	                ...
	                if (amount <= 0){
		                ...
		                printf(GREEN "Deposit successful! \n"YELLOW"New balance: %.2lf\n" RESET, user.AccountBalance);
		                
		                // Record the transaction
		                strcpy(transaction.AccountNumber, user.AccountNumber);
            ...
            case 3: // View Balance
            	printf(CYAN BOLD "========================================\n" RESET);
                printf(GREEN "\nYour total deposit: "WHITE"%.2lf\n", TotalDeposit);
                printf(YELLOW"Your total withdrawal: "WHITE"%.2lf\n", TotalWithdrawal);
                printf(CYAN"Your current balance: "WHITE"%.2lf\n" RESET, user.AccountBalance);
                break;
                ...
                while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
                    if (transaction.AccountNumber == AccountNumber) {
                        if (strcmp(transaction.TransactionCode, "D") == 0) {
                            printf(CYAN BOLD "|"GREEN" %15s "CYAN BOLD"|"YELLOW" %15.2lf+ "CYAN BOLD"|"WHITE" %10s "CYAN BOLD"|"WHITE" %8s "CYAN BOLD"|\n" RESET, "Deposit", transaction.Amount, transaction.DateSTR, transaction.TimeSTR);
                        } else if (strcmp(transaction.TransactionCode, "W") == 0) {
                            printf(CYAN BOLD "|"RED" %15s "CYAN BOLD"|"YELLOW" %15.2lf- "CYAN BOLD"|"WHITE" %10s "CYAN BOLD"|"WHITE" %8s "CYAN BOLD"|\n" RESET, "Withdrawal", transaction.Amount, transaction.DateSTR, transaction.TimeSTR);
                        }
                    }
                }
                printf(CYAN BOLD "--------------------------------------------------------------\n" RESET);
...
void ListTransact() {
	...
	while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
	    ...
	    if (strcmp(transaction.TransactionCode, "D") == 0) {
	        printf(WHITE " %-20s "MAGENTA"|"WHITE" %-25s "MAGENTA"| " GREEN "%-6s" MAGENTA "  |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Deposit", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    } else if (strcmp(transaction.TransactionCode, "W") == 0) {
	        printf(WHITE " %-20s "MAGENTA"|"WHITE" %-25s "MAGENTA"| " RED "%-6s" MAGENTA " |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Withdraw", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    } else {
	        printf(WHITE " %-20s "MAGENTA"|"WHITE" %-25s "MAGENTA"| " YELLOW "%-6s" MAGENTA " |"WHITE" %-15s "MAGENTA"|"WHITE" %-12s "MAGENTA"| " WHITE "%-8.2lf "MAGENTA"|\n" RESET, transaction.AccountNumber, fullName, "Unknown", transaction.DateSTR, transaction.TimeSTR, transaction.Amount);
	    }
	}
...
void FilterData() {
    char enteredAN[50] = "NULL"; 
    char enteredFN[50] = "NULL"; // John Mathew
    ...
    while (1) {
        ...
        printf(CYAN "1. Account Number   : %s\n", enteredAN);
...
void SearchData(char *AN, char *FN, char *LN, char *Month, char *Date, char *Year) {
    ...
    while (fread(&transaction, sizeof(struct TransactionData), 1, transFile)) {
        ...
        if (strcmp(AN, "NULL") != 0 && strcmp(transaction.AccountNumber, AN) != 0) {
            match = 0;
        }
        ...
        if (match == 1) {
            ...
            printf(WHITE " %-20s " MAGENTA "|" WHITE " %-25s " MAGENTA "| " WHITE "%-6s" MAGENTA " |" WHITE " %-15s " MAGENTA "|"WHITE" %-12s " MAGENTA "| " WHITE "%-10.2lf " MAGENTA "|\n" RESET,
